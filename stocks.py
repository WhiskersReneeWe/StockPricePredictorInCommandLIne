...
def split_sequence(sequence, n_steps):
    """sequence: get_data['PK'].values.flatten()"""
...
def split_sequence_mult(sequence, n_steps_in, n_steps_out):
    ...
    """sequence = get_data(['ticker_name']).values.flatten"""
    X, y = list(), list()
    for i in range(len(sequence)):
        # find the end of this pattern
        end_ix = i + n_steps_in
...
def reshape_data(price_X):
    ...
    """price_X -- reshaped prices containing history, target
               -- output from split_sequence or split_sequence_mult function"""
    
    num_samples, timesteps, num_features = price_X.shape[0], price_X.shape[1], 1 # Univariate time series, feature = 1, by default
    reshaped_data = price_X.reshape((num_samples, timesteps, num_features))
    
    return reshaped_data

def config_model(timesteps, num_features=1, n_next_days):
    
    model = Sequential()
    model.add(LSTM(units=32, activation='relu', return_sequences=True, input_shape=(timesteps, num_features)))
...
def train_lstm(tick_name, scaler_obj, timestep=10, epochs=150, n_next_days=7):
    ...
    model = config_model(timestep=tick_X.shape[1], num_features=1, n_next_days=7)
