...
def split_sequence_mult(sequence, n_steps_in, n_steps_out):
    ...
    """sequence = get_data(['ticker_name']).values"""
    X, y = list(), list()
    for i in range(len(sequence)):
        # find the end of this pattern
        end_ix = i + n_steps_in
...
def reshape_data(price_X):
    ...
    reshaped_data = price_X.reshape((num_samples, timesteps, num_feautures))
    
    return reshaped_data

def config_model(timesteps, num_features=1, n_next_days=1):
